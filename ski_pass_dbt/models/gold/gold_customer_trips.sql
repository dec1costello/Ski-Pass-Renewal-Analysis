...
with base as (
    select 
        ...
        rd.resort_state
    from {{ ref('silver_guest_demographics') }} gd
    inner join {{ ref('silver_guest_visitation') }} gv
        on gd.customer_key = gv.customer_key
    inner join {{ ref('silver_resort_dimensions') }} rd
        on gv.resort_key = rd.resort_key
    left join {{ ref('silver_guest_transactions') }} gt
